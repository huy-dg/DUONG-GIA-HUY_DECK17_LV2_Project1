...
def create_new_topic(admin, topic_config):
    ...
    logger.info(f"Existing topics: {list(topic_list)}")
    ...
    for topic, f in fs.items():
        try:
            ...
            if err.code() == KafkaError.TOPIC_ALREADY_EXISTS:
                ...
                logger.exception(f"❌ Failed to create topic '{topic}': {err.str()}")
...
def describe_topics(admin):
    ...
    for topic_name in list(topic_list):
        ...
        for pid, part in topic_info.partitions.items():
            ...
            isr = part.isrs
            logger.info(
                f"Partition {pid}: leader={leader}, replicas={replicas}, isr={isr}"
            )
    ...
    for topic, f in fs.items():
        try:
            ...
            if err.code() == KafkaError.UNKNOWN_TOPIC_OR_PART:
                ...
                logger.exception(f"❌ Failed to delete topic '{topic}': {err.str()}")
        except Exception as e:
            logger.exception(f"Error: {e}")
